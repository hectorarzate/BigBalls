...
#if defined(NDEBUG)
# error "Bigballs cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x9ac4ad6273b982363d54ac90f6e1754baf820e70295a259a410127397251680c");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Bigballs: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Bigballs Signed Message:\n";
...
{
    // Bigballs: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 69000); // Bigballs: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 1 * 24 * 60 * 60; // Bigballs: 1 days
static const int64 nTargetSpacing = 1 * 60; // Bigballs: 1 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Bigballs: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Bigballs: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x8cd24f7a23435e3911ae5cda5df7ceaef983cb31faac5931268a392e19d078bd");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "twitter 05/MAY/2021 Elon Musk, Exactly to Tim Urban";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 69 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("04e4b5b0dda7a3783be16109c64b95e1cc8376b6d30a81b636af88389573759329318efbd09c43ad4b9e584c12db38072d1d6c9ebcc6c3594ae35797b5cfea1e52") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1622166451;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2085110169;

        if (fTestNet)
        {
            block.nTime    = 1622166432;
            block.nNonce   = 387797501;
        }
...
if (false && block.GetHash() != hashGenesisBlock)
        ...
        assert(block.hashMerkleRoot == uint256("0x476b0bbcb75eb655370f6040cf45c5b7fe61c16d638674e71e16e73cd5452d9a"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xfb, 0xcb, 0xb1, 0xd6 }; // Bigballs: just change the last letter to bigballs bb16 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("BigballsMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("BigballsMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static BigballsMiner(CWallet *pwallet)
{
    printf("BigballsMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("bigballs-miner");
    ...
    try { loop {
        ...
        printf("Running BigballsMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("BigballsMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&BigballsMiner, pwallet));
}
